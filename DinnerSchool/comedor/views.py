...
def orderHistory(request):
    ...
    if request.user.is_authenticated:
        ...
        from core.models import Tutor, Alumnos, Empleados, Usuarios
        ...
        return render(request, 'Orders/orders_history_view.html', context)
                pedidos = Pedido.objects.filter(profesorId=profesor).order_by('-fecha', '-id')
            elif is_admin:
                pedidos = Pedido.objects.all().order_by('-fecha', '-id')
            else:
                messages.error(request, "No tienes permisos para ver el historial de pedidos.")
                return redirect('core:dashboard')
            ...
            for pedido in pedidos:
                pedidos_list.append({
                    'id': pedido.id,
                    'usuario': f"{pedido.alumnoId.nombre} {pedido.alumnoId.paterno}" if pedido.alumnoId else f"{pedido.profesorId.usuario.nombre} {pedido.profesorId.usuario.paterno}",
                    'platillo': pedido.platillo.nombre,
                    'cantidad': pedido.cantidad if hasattr(pedido, 'cantidad') else 1,
                    'turno': pedido.get_turno_label(),
                    'total': float(pedido.total) if isinstance(pedido.total, Decimal) else 0.00,
                    'fecha': pedido.fecha,
            ...
            page_obj = paginator.get_page(page_number)
            context = {
                'pedidos': page_obj # Aquí es donde pasas el objeto paginado a la plantilla
            }
            return render(request, 'Orders/orders_history_view.html', context)
        except Exception as e:
            print("Error en la vista OrderHistory:")
            print(f"Error: {str(e)}")
            traceback.print_exc()
            return render(request, 'Orders/orders_history_view.html', {'pedidos': []})   
